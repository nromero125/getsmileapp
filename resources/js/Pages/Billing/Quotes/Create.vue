...
const props = defineProps({ patients: Array, treatments: Array, patient_id: [String, Number] })
...
const applyTreatment = (i, id) => {
  const t = props.treatments.find(t => t.id == id)
  if (t) { form.items[i].description = t.name; form.items[i].unit_price = t.default_price; form.items[i].treatment_id = t.id }
}
  ...
  <AppLayout>
    <template #header>
      <div class="flex items-center gap-3">
        ...
        <h1 class="text-xl font-display font-semibold text-navy-900 dark:text-white">Nueva Cotización</h1>
      </div>
    </template>
    ...
    <form @submit.prevent="submit" class="max-w-3xl space-y-4">
      <div class="card p-6">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input v-model="form.quote_date" type="date" class="input" required />
          </div>
          <div>
      ...
      <div class="card p-6">
        ...
        <div class="space-y-3">
          <div v-for="(item, i) in form.items" :key="i"
            ...
            <div class="col-span-4 sm:col-span-2"><label class="label">Cant.</label><input v-model="item.quantity" type="number" min="1" class="input" /></div>
            <div class="col-span-4 sm:col-span-2"><label class="label">Precio</label><input v-model="item.unit_price" type="number" step="0.01" min="0" class="input" /></div>
            <div class="col-span-4 sm:col-span-2"><label class="label">Descuento</label><input v-model="item.discount" type="number" step="0.01" min="0" class="input" /></div>
            <div class="col-span-12 sm:col-span-1 flex items-end">
              <button type="button" @click="removeItem(i)" :disabled="form.items.length <= 1"
                class="btn-ghost p-1.5 text-red-400 hover:text-red-600 disabled:opacity-30">
      ...
      <div class="flex justify-end gap-3">
        <Link :href="route('quotes.index')" class="btn-outline">Cancelar</Link>
        <button type="submit" class="btn-primary" :disabled="form.processing">{{ form.processing ? 'Creando…' : 'Crear Cotización' }}</button>
      </div>
    </form>
  </AppLayout>
</template>
